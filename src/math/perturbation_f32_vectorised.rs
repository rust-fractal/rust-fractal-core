...
impl ImageRenderer {
    pub fn calculate_perturbations_f32_vectorised(&self, points_remaining: &mut Vec<Point<ComplexFixed<f32>>>, points_complete: &mut Vec<Point<ComplexFixed<f32>>>) {
        ...
        *points_remaining = points_remaining.chunks(lanes)
                                            ...
                                            .collect::<Vec<Vec<Point<ComplexFixed<f32>>>>>()
                                            .into_par_iter()
                                            .map(|chunk| {
                                                ...
                                                for iteration in 0..self.maximum_iterations {
                                                    ...
                                                    let new_z_norm = (ComplexVector::<f32x8>::splat( self.x_n_f32[iteration + 1]) + delta_n).norm_sqr();
                                                ...
                                                let smooth = escaped.select(nu, f32x8::splat(0.0));
