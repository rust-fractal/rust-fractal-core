...
impl ImageRenderer {
    ...
    pub fn calculate_perturbations_f32(&self, points_remaining: &mut Vec<Point<ComplexFixed<f32>>>, points_complete: &mut Vec<Point<ComplexFixed<f32>>>) {
        *points_remaining = points_remaining.into_par_iter()
            .map(|point| {
                let delta_0 = point.delta - self.reference_delta_f32;
                ...
                while z_norm < 256.0 && iteration < self.maximum_iterations {
                    let temp = delta_n;
                    delta_n += self.x_n_2_f32[iteration];
                    delta_n *= temp;
                    delta_n += delta_0;

                    iteration += 1;
                    z_norm = (self.x_n_f32[iteration] + delta_n).norm_sqr();

                    if z_norm < self.tolerance_check_f32[iteration] {
                        glitched = true;
                        break;
                    }
                }
